...
func RoundFloat64(val float64, prec int) float64 {
	...
	//观察下一位精度数值
	nextPrec := prec + 1
	nextVal := math.Pow10(-1 * nextPrec)
	//当前精度的数值
	divVal := math.Pow10(prec)
	addVal := math.Pow10(-1 * prec)
	//四舍五入处理
	tmp := math.Mod(math.Trunc(val/nextVal), 10)
	if tmp >= 5 {
		retVal = val + addVal
	} else {
		retVal = val
	}
	return math.Trunc(retVal/addVal) / divVal
}
